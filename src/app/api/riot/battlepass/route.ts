...
import { getBattlepass } from "@/lib/riotPrivateApi";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const battlepass = await getBattlepass(
      account.puuid,
      account.accessToken!,
      account.entitlementsToken!,
      account.region
    );
    return Response.json(battlepass ?? { contractId: null, message: "활성 배틀패스 없음" });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "알 수 없는 오류";
    console.error("Battlepass fetch error:", message);
    ...
    return Response.json(
      { error: `배틀패스 조회 중 오류가 발생했습니다. (${message})` },
      { status: 500 }
    );
  }
}
