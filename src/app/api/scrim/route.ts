...
export async function GET(req: NextRequest) {
  ...
  // Fetch user details and guild nicknames for tier info
  const userIds = kdRanking.map(p => p.userId);
  const [usersWithDetails, guildNicknames] = await Promise.all([
    prisma.user.findMany({
      ...
      select: {
        ...
        name: true,
        riotAccounts: {
          select: { cachedTierName: true, region: true },
        },
      },
    }),
    guild
  ...
  const nicknameMap = new Map(guildNicknames.map(m => [m.userId, m.nickname]));
  ...
  const finalKdRanking = kdRanking.slice(0, 20).map(player => {
    ...
    return {
      ...player,
      name: nicknameMap.get(player.userId) ?? userDetail?.name ?? player.name,
      tierName,
      tierIconUrl,
      region,
      regionLabel,
