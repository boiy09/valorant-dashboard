    ...
    <div>
      ...
      {writerOpen ? (
        <AnnouncementWriter
          title={title}
          content={content}
          pinned={pinNewAnnouncement}
          saving={saving}
          message={message}
          onTitleChange={setTitle}
          onContentChange={setContent}
          onPinnedChange={setPinNewAnnouncement}
          onClose={() => {
            setWriterOpen(false);
            resetWriter();
          }}
...
function EmptyCard({ text }: { text: string }) {
  return <div className="val-card flex min-h-[210px] items-center justify-center p-8 text-center text-sm text-[#7b8a96]">{text}</div>;
}
...
function NewsGrid({ items, loading }: { items: PatchNote[]; loading: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      {items.map((item) => (
        <a key={item.url} href={item.url} target="_blank" rel="noopener noreferrer" className="group val-card overflow-hidden transition-colors hover:border-[#ff4655]/70">
...
function VideoSection({ title, description, videos, loading }: { title: string; description: string; videos: VideoItem[]; loading: boolean }) {
  return (
    <section>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {videos.map((video) => (
            <a key={video.id} href={video.url} target="_blank" rel="noopener noreferrer" className="group val-card overflow-hidden transition-colors hover:border-[#ff4655]/70">
              <div className="relative aspect-video bg-black">
                <img src={video.thumbnail} alt={video.title} className="h-full w-full object-cover" />
                <span className="absolute inset-0 flex items-center justify-center bg-black/10 text-3xl font-black text-white/90">▶</span>
              </div>
              <div className="p-3">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="val-card w-full max-w-2xl p-5 shadow-2xl shadow-black/40">
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onClose} className="rounded border border-[#2a3540] px-3 py-1.5 text-xs font-bold text-[#c8d3db] hover:border-[#ff4655] hover:text-white">
            닫기
          </button>
        </div>

        <input value={title} onChange={(event) => onTitleChange(event.target.value)} placeholder="공지 제목" maxLength={80} className="mb-3 w-full rounded border border-[#2a3540] bg-[#07131e] px-3 py-2 text-sm font-bold text-white outline-none transition-colors placeholder:text-[#4a5a68] focus:border-[#ff4655]" />

        <div className="mb-2 flex flex-wrap gap-2">
          {FORMAT_HINTS.map((item) => (
            <button key={item.label} type="button" onClick={() => onInsertFormat(item.before, item.after)} className="rounded border border-[#2a3540] bg-[#0f1923] px-2.5 py-1 text-[11px] font-bold text-[#c8d3db] hover:border-[#ff4655] hover:text-white">
              {item.label}
            </button>
          ))}
        </div>

        <textarea value={content} onChange={(event) => onContentChange(event.target.value)} placeholder="공지 내용을 입력하세요." rows={8} maxLength={2000} className="w-full resize-y rounded border border-[#2a3540] bg-[#07131e] px-3 py-2 text-sm leading-relaxed text-white outline-none transition-colors placeholder:text-[#4a5a68] focus:border-[#ff4655]" />
        ...
        <div className="mt-3 rounded border border-[#2a3540] bg-[#07131e]/70 p-3">
          ...
          <div className="max-h-44 overflow-y-auto pr-2 whitespace-pre-wrap text-sm leading-relaxed text-[#c8d3db]">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <label className="flex items-center gap-2 text-xs font-bold text-[#c8d3db]">
            <input type="checkbox" checked={pinned} onChange={(event) => onPinnedChange(event.target.checked)} className="h-4 w-4 accent-[#ff4655]" />
            상단 고정
          </label>
          <div className="flex items-center gap-3">
            <span className="text-xs text-[#7b8a96]">{message}</span>
            <button type="button" onClick={onSubmit} disabled={saving} className="rounded bg-[#ff4655] px-4 py-2 text-xs font-black text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-50">
              {saving ? "등록 중..." : "공지 등록"}
            </button>
          </div>
        </div>
...
function renderInlineFormat(value: string, keyPrefix: string) {
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith("**") && part.endsWith("**")) return <strong key={key} className="font-black text-white">{part.slice(2, -2)}</strong>;

    const match = part.match(/^\[(red|green|yellow|large)\]([\s\S]*)\[\/\1\]$/);
    if (!match) return <Fragment key={key}>{part}</Fragment>;

    const [, type, text] = match;
    const className = type === "red" ? "font-black text-[#ff4655]" : type === "green" ? "font-black text-emerald-400" : type === "yellow" ? "font-black text-amber-300" : "text-base font-black text-white";
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-[#7b8a96]">
              ...
              <span role="button" tabIndex={0} onClick={copyId} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); copyId(event); } }} title="공지 ID 복사" className="cursor-copy rounded border border-[#2a3540] bg-[#0f1923] px-1.5 py-0.5 font-mono text-[10px] text-[#c8d3db] transition-colors hover:border-[#ff4655] hover:text-white">
