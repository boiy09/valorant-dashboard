            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1">
                <div className="text-2xl font-black text-white">
                  {result.gameName}<span className="text-[#7b8a96] font-normal text-lg">#{result.tagLine}</span>
                </div>
                {result.rank ? (
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    <span className="text-[#7b8a96] text-sm">{result.rank.wins}승 / {result.rank.games - result.rank.wins}패</span>
                  </div>
                ) : (
                  ...
                  return (
                    <div key={i} className="val-card px-5 py-3 flex items-center gap-4"
                      ...
                      <div className="text-[#7b8a96] text-xs flex-shrink-0">
                        {new Date(m.playedAt).toLocaleDateString("ko-KR", { month: "short", day: "numeric" })}
                      </div>
                    </div>
                  );
                })}
