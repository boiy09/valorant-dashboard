...
function readStatsCache() {
  ...
  try {
    const raw = window.sessionStorage.getItem("valorant-dashboard:valorant-stats:v2");
    if (!raw) return null;
...
function writeStatsCache(data: { accounts: RegionStats[] }) {
  ...
  try {
    window.sessionStorage.setItem("valorant-dashboard:valorant-stats:v2", JSON.stringify({ savedAt: Date.now(), data }));
  } catch {}
}
